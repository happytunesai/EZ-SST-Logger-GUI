...
def load_config(config_path, key):
    ...
    defaults = {
        ...
        "language_ui": DEFAULT_LANGUAGE, # Sprache für GUI
        "log_level": DEFAULT_LOG_LEVEL, # NEU: Log-Level hinzugefügt
    ...
    # Validiere Sprache und Log-Level
    if config.get("language_ui") not in SUPPORTED_LANGUAGES:
        logger.warning(f"Ungültiger Sprachcode '{config.get('language_ui')}' in Config gefunden. Setze auf Standard '{DEFAULT_LANGUAGE}'.")
        config["language_ui"] = DEFAULT_LANGUAGE
    # FIX: Validiere Log-Level gegen die Liste der gültigen Namen
...
def save_config(config_path, config_dict, key):
    ...
    try:
        ...
        # FIX: Stelle sicher, dass der Log-Level gültig ist, bevor gespeichert wird
        if config_to_save.get("log_level") not in LOG_LEVEL_NAMES:
        ...
        elif "log_level" not in config_to_save: # Füge hinzu, falls es fehlt
             config_to_save["log_level"] = DEFAULT_LOG_LEVEL
         ...
         # Setze Log-Level auf Default bei Konvertierungsfehler
         config_to_save["log_level"] = DEFAULT_LOG_LEVEL
