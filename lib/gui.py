...
Implements internationalization (i18n) and log level control.
Enables dynamic starting/stopping of the WebSocket server.
"""
...
class WhisperGUI(ctk.CTk):
    ...
    def __init__(self, app_config, key, queues, flags, handlers):  # Add handlers
        """Initializes the GUI."""
        super().__init__()
        ...
        self._start_background_tasks()  # Starts only based on initial configuration
    ...
    def _setup_window(self):
        """Configures the main window of the application."""
        logger.debug(tr("log_gui_setup_window"))
        try:
            if os.path.exists(ICON_FILE): 
                self.iconbitmap(ICON_FILE)
            else: 
                logger.warning(tr("log_gui_icon_not_found", icon_file=ICON_FILE))
        except Exception as e: 
            logger.warning(tr("log_gui_icon_error", error=e))
    ...
    def _create_widgets(self):
        """Creates and arranges all widgets in the GUI."""
    ...
    def _create_tab_view(self):
        """Creates the tab view and its content."""
        ...
        try:
            self.tab_view.set(tr(initial_tab_key))
            logger.debug(tr("log_gui_initial_tab_set", tab_name=tr(initial_tab_key)))
        except Exception as e:
             logger.warning(tr("log_gui_initial_tab_error", tab_name=tr(initial_tab_key), error=e))
             try: 
                 self.tab_view.set(tr("tab_local"))
    ...
    def _create_websocket_tab(self, tab):
        ...
        # FIX: Checkbox now receives a command
        self.websocket_enable_checkbox = ctk.CTkCheckBox(
            tab,
            text=tr("checkbox_websocket_enable"),
            command=self._on_websocket_enable_change  # NEW callback function for WebSocket checkbox
        )
    ...
    def _create_integration_tab(self, tab):
        ...
        self.streamerbot_ws_enable_checkbox = ctk.CTkCheckBox(
            tab,
            text=tr("checkbox_integration_enable"),
            command=self._on_streamerbot_enable_change  # NEW callback function for Streamer.bot checkbox
        )
    ...
    def _create_common_config_widgets(self):
        ...
        if self.config.get("clear_log_on_start", False): 
            self.clear_log_checkbox.select()
    ...
    def _create_language_selector(self):
        ...
        if current_display_language not in language_options:
             logger.warning(tr("log_gui_language_not_found", language=current_display_language))
    ...
    def _start_background_tasks(self):
        ...
        # Start WS server only if enabled in config (dynamic start via checkbox)
        if self.config.get("websocket_enabled", False): 
            self._start_websocket_server()
        # Start SB client only if enabled in config (dynamic start via checkbox)
        if self.config.get("streamerbot_ws_enabled", False): 
    ...
    # NEW callback function for log level
    ...
    # NEW callback function for WebSocket checkbox
    def _on_websocket_enable_change(self):
        ...
        self.config["websocket_enabled"] = is_enabled  # Update config immediately
        if is_enabled:
            self._start_websocket_server()
        else:
            self._stop_websocket_server()
        self._update_initial_status()  # Update status line

    # NEW callback function for Streamer.bot checkbox
    def _on_streamerbot_enable_change(self):
        ...
        self.config["streamerbot_ws_enabled"] = is_enabled  # Update config immediately
        if is_enabled:
            self._start_streamerbot_client()
        else:
    ...
    # ... (Methods from _update_initial_status to on_closing) ...
    ...
    def _edit_filter_file(self):
        ...
        if not os.path.exists(target_file): 
            load_filter_patterns(target_file)
        if not os.path.exists(target_file):
    ...
    def _check_record_button_state(self):
        try:
            ...
            if self.is_recording:
                ...
                if current_mode in ["websocket", "integration"]: 
                    self.start_stop_button.configure(state="disabled", text=stop_text)
                else: 
                    self.start_stop_button.configure(state="normal", text=stop_text)
            else:
                self.indicator_light.configure(fg_color="grey")
                start_text = tr("button_start_recording")
                if current_mode in ["websocket", "integration"]: 
                    self.start_stop_button.configure(state="disabled", text=start_text)
                else: 
                    self.start_stop_button.configure(state="normal", text=start_text)
            self.after(500, self._check_record_button_state)
        except Exception as e:
             if isinstance(e, tk.TclError) and "invalid command name" in str(e):
                 logger.debug(tr("log_gui_tkerror_check_button"))
             else:
    ...
    def _validate_start_conditions(self):
        ...
        if not mic_name or mic_name == tr("combobox_mic_loading") or mic_name == tr("combobox_mic_nodata") or mic_name not in self.available_mics:
            self._update_status("status_error_mic_select_fail", level="error", is_key=True)
            return False
        ...
        if processing_mode == "openai" and not self.openai_api_key_entry.get():
            self._update_status("status_error_api_key_openai", level="error", is_key=True)
            return False
        if processing_mode == "elevenlabs":
             if not self.elevenlabs_api_key_entry.get():
                 self._update_status("status_error_api_key_elevenlabs", level="error", is_key=True)
                 return False
             if not self.elevenlabs_model_id_entry.get():
                 self._update_status("status_error_model_id_elevenlabs", level="error", is_key=True)
                 return False
        try: 
    ...
    def _gather_runtime_config_dict(self):
         try:
             config_dict = {
                 ...
                 "log_level": self.config.get("log_level", DEFAULT_LOG_LEVEL),  # Add log level
    ...
    def _prepare_worker_args(self, current_config):
        ...
        if args["device_id"] is None: 
            raise ValueError(tr("log_gui_error_mic_id_not_found"))
    ...
    def _update_status(self, message_or_key, level="info", log=True, is_key=True, **kwargs):
        if is_key:
            ...
            try: 
                message = str(message_or_key).format(**kwargs) if kwargs else str(message_or_key)
            except: 
                message = str(message_or_key)
    ...
    def _start_websocket_server(self):
        ...
        if self.websocket_server_thread and self.websocket_server_thread.is_alive(): 
            return
        try: 
            port = int(self.websocket_port_entry.get())
    ...
    def _start_streamerbot_client(self):
        if not self.config.get("streamerbot_ws_enabled", False): 
            return
    ...
    def _tab_name_to_mode_safe(self, tab_name):
        """
        Converts the *initial* GUI tab name (from the default language)
        to an internal mode ID using the fixed map.
        """
        ...
        if self.is_recording:
            ...
            if self.recording_thread and self.recording_thread.is_alive():
                ...
                if self.recording_thread.is_alive():
                    logger.warning(tr("log_gui_recording_thread_timeout"))
                else:
                    logger.info(tr("log_gui_recording_thread_terminated"))
            else:
                logger.debug(tr("log_gui_no_active_recording_thread"))
            self.is_recording = False
